...
use darth_lsp::LspContextEnrichment;
...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            ...
            dir.join(".darth").join("DARTH.md"),
            dir.join(".darth").join("instructions.md"),
        ] {
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".darth")).expect("nested darth dir");
        fs::write(root.join("DARTH.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("CLAW.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".darth")).expect("apps darth dir");
        fs::write(root.join("apps").join("DARTH.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".darth").join("instructions.md"),
            "apps dot darth instructions",
        )
        .expect("write apps dot darth instructions");
        fs::write(nested.join(".darth").join("DARTH.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".darth").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("DARTH.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("DARTH.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.darth/DARTH.md")),
            "DARTH.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("DARTH.md"), "rules").expect("write instructions");
    ...
    #[test]
    fn load_system_prompt_reads_darth_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".darth")).expect("darth dir");
        fs::write(root.join("DARTH.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".darth").join("settings.json"),
        ...
        let original_darth_home = std::env::var("DARTH_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("DARTH_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_darth_home {
            std::env::set_var("DARTH_CONFIG_HOME", value);
        } else {
            std::env::remove_var("DARTH_CONFIG_HOME");
        }
    ...
    #[test]
    fn renders_darth_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".darth")).expect("darth dir");
        fs::write(root.join("DARTH.md"), "Project rules").expect("write CLAW.md");
        fs::write(
            root.join(".darth").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
    ...
    #[test]
    fn discovers_dot_darth_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".darth")).expect("nested darth dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".darth/instructions.md")));
        assert!(
