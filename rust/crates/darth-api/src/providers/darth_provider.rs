...
fn resolve_saved_oauth_token_set(
    ...
    let client = DarthApiClient::from_auth(AuthSource::None).with_base_url(read_base_url());
...
impl Provider for DarthApiClient {
...
mod tests {
    ...
    use darth_runtime::{clear_oauth_credentials, save_oauth_credentials, OAuthConfig};

    use super::{
        now_unix_timestamp, oauth_token_is_expired, resolve_saved_oauth_token,
        resolve_startup_auth_source, AuthSource, DarthApiClient, OAuthTokenSet,
    };
    ...
    fn auth_source_from_saved_oauth_when_env_absent() {
        ...
        std::env::set_var("DARTH_CONFIG_HOME", &config_home);
        std::env::remove_var("ANTHROPIC_AUTH_TOKEN");
        std::env::remove_var("ANTHROPIC_API_KEY");
        save_oauth_credentials(&darth_runtime::OAuthTokenSet {
    ...
    fn resolve_saved_oauth_token_refreshes_expired_credentials() {
        ...
        let stored = darth_runtime::load_oauth_credentials()
    ...
    fn backoff_doubles_until_maximum() {
        let client = DarthApiClient::new("test-key").with_retry_policy(
            3,
