...
pub fn mvp_tool_specs() -> Vec<ToolSpec> {
    vec![
        ...
        ToolSpec {
            name: "Config",
            description: "Get or set Darth Agent settings.",
            input_schema: json!({
                "type": "object",
                "properties": {
                    "setting": { "type": "string" },
...
fn build_http_client() -> Result<Client, String> {
    Client::builder()
        ...
        .user_agent("darth-rust-tools/0.1")
        .build()
        .map_err(|error| error.to_string())
}
...
fn spawn_agent_job(job: AgentJob) -> Result<(), String> {
    let thread_name = format!("darth-agent-{}", job.manifest.agent_id);
...
fn agent_store_dir() -> Result<std::path::PathBuf, String> {
    ...
    if let Some(workspace_root) = cwd.ancestors().nth(2) {
        return Ok(workspace_root.join(".darth-agents"));
    }
    Ok(cwd.join(".darth-agents"))
}
...
fn normalize_subagent_type(subagent_type: Option<&str>) -> String {
    ...
    match canonical_tool_token(trimmed).as_str() {
        ...
        "darthguide" | "darthguideagent" | "guide" => String::from("darth-guide"),
...
fn config_file_for_scope(scope: ConfigScope) -> Result<PathBuf, String> {
    ...
    Ok(match scope {
        ...
        ConfigScope::Settings => cwd.join(".darth").join("settings.local.json"),
    })
}

fn config_home_dir() -> Result<PathBuf, String> {
    if let Ok(path) = std::env::var("DARTH_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var("HOME").map_err(|_| String::from("HOME is not set"))?;
    Ok(PathBuf::from(home).join(".darth"))
}
...
#[allow(clippy::needless_pass_by_value)]
fn execute_powershell(input: PowerShellInput) -> std::io::Result<darth_runtime::BashCommandOutput> {
...
fn execute_shell_command(
    ...
    timeout: Option<u64>,
    run_in_background: Option<bool>,
) -> std::io::Result<darth_runtime::BashCommandOutput> {
    if run_in_background.unwrap_or(false) {
        let child = std::process::Command::new(shell)
            ...
            .spawn()?;
        return Ok(darth_runtime::BashCommandOutput {
            stdout: String::new(),
            stderr: String::new(),
...
Command exceeded timeout of {timeout_ms} ms",
    ...
    Ok(darth_runtime::BashCommandOutput {
        stdout: String::from_utf8_lossy(&output.stdout).into_owned(),
        stderr: String::from_utf8_lossy(&output.stderr).into_owned(),
        raw_output_path: None,
        interrupted: false,
...
mod tests {
    ...
    use darth_api::OutputContentBlock;
    use darth_runtime::{ApiRequest, AssistantEvent, ConversationRuntime, RuntimeError, Session};
    use serde_json::json;
    ...
    fn temp_path(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .expect("time")
            .as_nanos();
        std::env::temp_dir().join(format!("darth-tools-{unique}-{name}"))
    }
    ...
    impl darth_runtime::ApiClient for MockSubagentApiClient {
        fn stream(&mut self, request: ApiRequest) -> Result<Vec<AssistantEvent>, RuntimeError> {
            self.calls += 1;
            match self.calls {
                1 => {
    ...
    fn subagent_runtime_executes_tool_loop_with_isolated_session() {
        ...
        assert!(runtime
            ...
            .any(|block| matches!(
                block,
                darth_runtime::ContentBlock::ToolResult { output, .. }
  ...
  "nbformat_minor": 5
    ...
    fn config_reads_and_writes_supported_values() {
        ...
        std::fs::create_dir_all(home.join(".darth")).expect("home dir");
        std::fs::create_dir_all(cwd.join(".darth")).expect("cwd dir");
        std::fs::write(
            home.join(".darth").join("settings.json"),
            r#"{"verbose":false}"#,
        )
        .expect("write global settings");

        let original_home = std::env::var("HOME").ok();
        let original_config_home = std::env::var("DARTH_CONFIG_HOME").ok();
        let original_dir = std::env::current_dir().expect("cwd");
        ...
        match original_config_home {
            Some(value) => std::env::set_var("DARTH_CONFIG_HOME", value),
            None => std::env::remove_var("DARTH_CONFIG_HOME"),
        }
