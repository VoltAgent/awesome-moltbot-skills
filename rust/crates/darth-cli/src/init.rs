...
const STARTER_DARTH_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_COMMENT: &str = "# Darth Agent local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".darth/settings.local.json", ".darth/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let darth_dir = cwd.join(".darth");
    artifacts.push(InitArtifact {
        name: ".darth/",
        status: ensure_dir(&darth_dir)?,
    });

    let darth_json = cwd.join(".darth.json");
    artifacts.push(InitArtifact {
        name: ".darth.json",
        status: write_file_if_missing(&darth_json, STARTER_DARTH_JSON)?,
    });
    ...
    let darth_md = cwd.join("DARTH.md");
    let content = render_init_darth_md(cwd);
    artifacts.push(InitArtifact {
        name: "DARTH.md",
        status: write_file_if_missing(&darth_md, &content)?,
    });
...
pub(crate) fn render_init_darth_md(cwd: &Path) -> String {
    let detection = detect_repo(cwd);
    let mut lines = vec![
        "# DARTH.md".to_string(),
        String::new(),
        "This file provides guidance to Darth Agent (darth-agency.dev) when working with code in this repository.".to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.darth.json`; reserve `.darth/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `DARTH.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
#[cfg(test)]
mod tests {
    use super::{initialize_repo, render_init_darth_md};
    use std::fs;
    use std::path::Path;
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("darth-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".darth/          created"));
        assert!(rendered.contains(".darth.json      created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("DARTH.md         created"));
        assert!(root.join(".darth").is_dir());
        assert!(root.join(".darth.json").is_file());
        assert!(root.join("DARTH.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".darth.json")).expect("read darth json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".darth/settings.local.json"));
        assert!(gitignore.contains(".darth/sessions/"));
        let darth_md = fs::read_to_string(root.join("DARTH.md")).expect("read darth md");
        assert!(darth_md.contains("Languages: Rust."));
        assert!(darth_md.contains("cargo clippy --workspace --all-targets -- -D warnings"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("DARTH.md"), "custom guidance\n").expect("write existing darth md");
        fs::write(root.join(".gitignore"), ".darth/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("DARTH.md         skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".darth/          skipped (already exists)"));
        assert!(second_rendered.contains(".darth.json      skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("DARTH.md         skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("DARTH.md")).expect("read existing darth md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".darth/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".darth/sessions/").count(), 1);
    ...
    fn render_init_template_mentions_detected_python_and_nextjs_markers() {
        ...
        let rendered = render_init_darth_md(Path::new(&root));
