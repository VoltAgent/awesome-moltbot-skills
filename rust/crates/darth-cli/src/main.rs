...
use darth_api::{
    resolve_startup_auth_source, AuthSource, DarthApiClient, ContentBlockDelta, InputContentBlock,
...
use darth_compat::{extract_manifest, UpstreamPaths};
use init::initialize_repo;
use darth_plugins::{PluginManager, PluginManagerConfig};
...
use serde_json::json;
use darth_tools::GlobalToolRegistry;
...
fn render_cli_error(problem: &str) -> String {
    ...
    lines.push("  Help             darth --help".to_string());
    lines.join("\n")
}
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // Darth Agent compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Darth Agent compat: --print makes output non-interactive
...
fn format_direct_slash_command_error(command: &str, is_unknown: bool) -> String {
    ...
    if is_unknown {
        ...
        lines.push("  Try              Start `darth` to use interactive slash commands".to_string());
        lines.push(
            "  Tip              Resume-safe commands also work with `darth --resume SESSION.json ...`"
                .to_string(),
        );
    }
    lines.join("\n")
...
fn print_bootstrap_plan() {
    for phase in darth_runtime::BootstrapPlan::darth_default().phases() {
        println!("- {phase:?}");
    }
}
...
fn default_oauth_config() -> OAuthConfig {
    OAuthConfig {
        ...
        scopes: vec![
            ...
            String::from("user:sessions:darth_agent"),
        ],
    }
}
...
fn run_login() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let redirect_uri = darth_runtime::loopback_redirect_uri(callback_port);
    ...
    let client = DarthApiClient::from_auth(AuthSource::None).with_base_url(darth_api::read_base_url());
    let exchange_request =
        OAuthTokenExchangeRequest::from_config(oauth, code, state, pkce.verifier, redirect_uri);
    let runtime = tokio::runtime::Runtime::new()?;
    let token_set = runtime.block_on(client.exchange_oauth_code(oauth, &exchange_request))?;
    save_oauth_credentials(&darth_runtime::OAuthTokenSet {
...
fn wait_for_oauth_callback(
    port: u16,
) -> Result<darth_runtime::OAuthCallbackParams, Box<dyn std::error::Error>> {
...
fn run_resume_command(
    ...
    match command {
        ...
        SlashCommand::Compact => {
            let result = darth_runtime::compact_session(
                session,
                CompactionConfig {
...
impl LiveCli {
    ...
    fn startup_banner(&self) -> String {
        ...
        let has_darth_md = cwd
            .as_ref()
            .is_some_and(|path| path.join("DARTH.md").is_file());
        let mut lines = vec![
            format!(
                "{} {}",
                if color {
                    "\x1b[1;38;5;45m🦞 Darth Agent\x1b[0m"
                } else {
                    "Darth Agent"
                },
                if color {
        ...
        if !has_darth_md {
            lines.push(
                "  First run        /init scaffolds DARTH.md, .darth.json, and local session files"
                    .to_string(),
            );
        }
        lines.join("\n")
    ...
    fn run_commit(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let path = write_temp_text_file("darth-commit-message.txt", &message)?;
    ...
    fn run_pr(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("darth-pr-body.md", &body)?;
    ...
    fn run_issue(&self, context: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if command_exists("gh") {
            let body_path = write_temp_text_file("darth-issue-body.md", &body)?;
            let output = Command::new("gh")
...
fn sessions_dir() -> Result<PathBuf, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let path = cwd.join(".darth").join("sessions");
...
fn init_darth_md() -> Result<String, Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    Ok(initialize_repo(&cwd)?.render())
}

fn run_init() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", init_darth_md()?);
    Ok(())
}
...
) -> Result<(darth_runtime::RuntimeFeatureConfig, GlobalToolRegistry), Box<dyn std::error::Error>> {
    let cwd = env::current_dir()?;
    let loader = ConfigLoader::default_for(&cwd);
    let runtime_config = loader.load()?;
    let plugin_manager = build_plugin_manager(&cwd, &loader, &runtime_config);
...
fn build_plugin_manager(
    cwd: &Path,
    loader: &ConfigLoader,
    runtime_config: &darth_runtime::RuntimeConfig,
) -> PluginManager {
...
impl darth_runtime::PermissionPrompter for CliPermissionPrompter {
    fn decide(
        &mut self,
        request: &darth_runtime::PermissionRequest,
    ) -> darth_runtime::PermissionPromptDecision {
        println!();
        ...
        match io::stdin().read_line(&mut response) {
            Ok(_) => {
                ...
                if matches!(normalized.as_str(), "y" | "yes") {
                    darth_runtime::PermissionPromptDecision::Allow
                } else {
                    darth_runtime::PermissionPromptDecision::Deny {
                        reason: format!(
            ...
            Err(error) => darth_runtime::PermissionPromptDecision::Deny {
...
impl DefaultRuntimeClient {
    fn new(
        ...
        Ok(Self {
            ...
            client: DarthApiClient::from_auth(resolve_cli_auth_source()?)
                .with_base_url(darth_api::read_base_url()),
            model,
            enable_tools,
            emit_output,
            allowed_tools,
...
fn resolve_cli_auth_source() -> Result<AuthSource, Box<dyn std::error::Error>> {
    Ok(resolve_startup_auth_source(|| {
        let cwd = env::current_dir().map_err(darth_api::ApiError::from)?;
        let config = ConfigLoader::default_for(&cwd).load().map_err(|error| {
            darth_api::ApiError::Auth(format!("failed to load runtime OAuth config: {error}"))
        })?;
...
fn final_assistant_text(summary: &darth_runtime::TurnSummary) -> String {
    summary
        .assistant_messages
        .last()
        .map(|message| {
...
fn collect_tool_uses(summary: &darth_runtime::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .assistant_messages
        .iter()
...
fn collect_tool_results(summary: &darth_runtime::TurnSummary) -> Vec<serde_json::Value> {
    summary
        .tool_results
        .iter()
        .flat_map(|message| message.blocks.iter())
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    writeln!(out, "Darth Agent CLI v{VERSION}")?;
    writeln!(
        out,
    ...
    writeln!(
        out,
        "  darth                                  Start the interactive REPL"
    )?;
    writeln!(
        out,
        "  darth \"summarize this repo\"            Run one prompt and exit"
    )?;
    writeln!(
        out,
        "  darth prompt \"explain src/main.rs\"     Explicit one-shot prompt"
    )?;
    writeln!(
        out,
        "  darth --resume SESSION.json /status    Inspect a saved session"
    )?;
    writeln!(out)?;
    ...
    writeln!(
        out,
        "  darth dump-manifests                   Read upstream TS sources and print extracted counts"
    )?;
    writeln!(
        out,
        "  darth bootstrap-plan                   Print the bootstrap phase skeleton"
    )?;
    writeln!(
        out,
        "  darth agents                           List configured agents"
    )?;
    writeln!(
        out,
        "  darth skills                           List installed skills"
    )?;
    writeln!(out, "  darth system-prompt [--cwd PATH] [--date YYYY-MM-DD]")?;
    writeln!(
        out,
        "  darth login                            Start the OAuth login flow"
    )?;
    writeln!(
        out,
        "  darth logout                           Clear saved OAuth credentials"
    )?;
    writeln!(
        out,
        "  darth init                             Scaffold DARTH.md + local files"
    )?;
    writeln!(out)?;
    writeln!(out, "Flags")?;
    writeln!(
    ...
    writeln!(out, "  darth --model opus \"summarize this repo\"")?;
    writeln!(
        out,
        "  darth --output-format json prompt \"explain src/main.rs\""
    )?;
    writeln!(
        out,
        "  darth --allowedTools read,glob \"summarize Cargo.toml\""
    )?;
    writeln!(
        out,
        "  darth --resume session.json /status /diff /export notes.txt"
    )?;
    writeln!(out, "  darth agents")?;
    writeln!(out, "  darth /skills")?;
    writeln!(out, "  darth login")?;
    writeln!(out, "  darth init")?;
    Ok(())
}
...
mod tests {
    ...
    use darth_api::{MessageResponse, OutputContentBlock, Usage};
    use darth_plugins::{PluginTool, PluginToolDefinition, PluginToolPermission};
    use darth_runtime::{AssistantEvent, ContentBlock, ConversationMessage, MessageRole, PermissionMode};
    use serde_json::json;
    ...
    fn shared_help_uses_resume_annotation_copy() {
        let help = darth_commands::render_slash_command_help();
        assert!(help.contains("Slash commands"));
        assert!(help.contains("Tab completes commands inside the REPL."));
        assert!(help.contains("available via darth --resume SESSION.json"));
    }
    ...
    fn cost_report_uses_sectioned_layout() {
        let report = format_cost_report(darth_runtime::TokenUsage {
            input_tokens: 20,
            output_tokens: 8,
    ...
    fn init_help_mentions_direct_subcommand() {
        ...
        assert!(help.contains("darth init"));
        assert!(help.contains("darth agents"));
        assert!(help.contains("darth skills"));
        assert!(help.contains("darth /skills"));
    }
    ...
    fn status_line_reports_model_and_token_totals() {
        let status = format_status_report(
            ...
            StatusUsage {
                message_count: 7,
                turns: 3,
                latest: darth_runtime::TokenUsage {
                    input_tokens: 5,
                    output_tokens: 4,
                    cache_creation_input_tokens: 1,
                    cache_read_input_tokens: 0,
                },
                cumulative: darth_runtime::TokenUsage {
                    input_tokens: 20,
                    output_tokens: 8,
 ...
 M src/main.rs",
    ...
    fn init_template_mentions_detected_rust_workspace() {
        let rendered = crate::init::render_init_darth_md(std::path::Path::new("."));
        assert!(rendered.contains("# DARTH.md"));
    ...
    fn ultraplan_progress_lines_include_phase_step_and_elapsed_status() {
        let snapshot = InternalPromptProgressState {
            ...
            detail: Some("reading rust/crates/darth-cli/src/main.rs".to_string()),
            saw_final_text: false,
        };
    ...
    fn describe_tool_progress_summarizes_known_tools() {
        ...
        assert!(
            describe_tool_progress("bash", r#"{"command":"cargo test -p darth-cli"}"#)
                .contains("cargo test -p darth-cli")
        );
        assert_eq!(
