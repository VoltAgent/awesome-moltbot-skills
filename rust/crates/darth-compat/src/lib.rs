...
use darth_commands::{CommandManifestEntry, CommandRegistry, CommandSource};
use darth_runtime::{BootstrapPhase, BootstrapPlan};
use darth_tools::{ToolManifestEntry, ToolRegistry, ToolSource};
...
fn upstream_repo_candidates(primary_repo_root: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in primary_repo_root.ancestors().take(4) {
        candidates.push(ancestor.join("darth-agent"));
    }

    candidates.push(primary_repo_root.join("reference-source").join("darth-agent"));
    candidates.push(primary_repo_root.join("vendor").join("darth-agent"));
